...
DBImpl::~DBImpl() {
  ...
  //5.4 删除无效文件
  DeleteObsoleteFiles();
  uint64_t gb = 1024*1024*1024;   //5.10
  Log(options_.info_log,  "Total_writes %lf GB; Total_creads %lf GB; Total_cwrites %lf GB",
      static_cast<double>(total_write)/gb,   
      static_cast<double>(creads)/gb,
      static_cast<double>(cwrites)/gb);
  if (db_lock_ != NULL) {
...
void DBImpl::DeleteObsoleteLdb(std::vector<uint64_t> numbers)
{
  ...
  //std::string filename = fn;
  //std::cout <<  filename << std::endl;
    env_->DeleteFile(dbname_ + "/" + fn + ".sst");
  }
...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  mutex_.AssertHeld();
  Log(options_.info_log,  "Compacted %lu@%d + %lu@%d files : %lld bytes => %lld bytes",
      compact->compaction->num_input_files(0),
      compact->compaction->level(),
      compact->compaction->num_input_files(1),
      compact->compaction->level() + 1,
      static_cast<long long>(compact->compaction->reads()),   //5.10
      static_cast<long long>(compact->total_bytes));

  cwrites += compact->total_bytes;
  creads += compact->compaction->reads();
  total_write += compact->total_bytes;  //5.10
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  if (compact->compaction->level() == 0){
    assert(compact->compaction->par() < config::kNumPars);
    Log(options_.info_log,  "Compacting %lu@%d:%d + %lu@%d files",
      compact->compaction->num_input_files(0),
