...
static uint64_t MaxFileSizeForLevel(unsigned level) {
  assert(level < leveldb::config::kNumLevels);
  static const uint64_t bytes[] = {32 * 1048576,
                                   32 * 1048576,
                                   32 * 1048576,
                                   32 * 1048576,
                                   32 * 1048576,
...
void VersionSet::Finalize(Version* v) {
  ...
  for (unsigned level = 0; level + 1 < config::kNumLevels; ++level) {
    ...
    if (level == 0) {
      ...
      //3.29 改变L0 compaction条件. set
      //5.3由于每次compaction都会有finalize，因此只选取下次LO compact时的分区号
      //int max = -1;
      //int temp_par = -1;
      ...
      /*
      if (temp_par != -1){
        v->l0_compact_indexes.push_back(temp_par);
      }
      */
  ...
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
	  c->read_bytes += files[i]->file_size;  //5.10
	 // std::cout << "fs" << files[i]->file_size << '\n';
          //std::cout << "rb" <<  c->read_bytes << '\n';
        ...
        const std::vector<FileMetaData*>& files = c->inputs_[which];
        for (size_t i = 0; i < files.size(); i++){
          c->read_bytes += files[i]->file_size;    //5.10 统计每次compaction输入的文件大小
        }
...
unsigned VersionSet::PickCompactionLevel(bool* locked, bool seek_driven) const {
  ...
  */
  //3.29 当L0 压缩索引集非空时
  if (!locked[0] && !locked[1] &&
      !current_->l0_compact_indexes.empty() &&
      current_->compaction_scores_[1] <= 1.0) {
    level = 0;
  }
  return level;
...
Compaction* VersionSet::PickCompaction(Version* v, unsigned level) {
  ...
  if (level > 0) {
    ...
    //4.26 L0compaction文件选择，分为两种情况，写导致的L0com和读导致的L0com
    //assert(!v->l0_compact_indexes.empty());
    //std::cout << "pick files:" << '\n';
    if (v->l0_compact_indexes.empty()){
      //4.26 read compaction
      size_t i = 0;
      for (; i < config::kNumPars; i++){
        int num = v->l0files[i].size();
        if (num > 0){
          std::vector<FileMetaData*> tmp(v->l0files[i]);
          if (num > 1){
            std::sort(tmp.begin(), tmp.end(), OldestFirst);
          }
         c->par_ = i; 
          for (size_t j = 0; j < tmp.size() && c->inputs_[0].size() < 32; ++j) {
              c->inputs_[0].push_back(tmp[j]);
          }
          break;
        }
      }
      ...
      int par = it->second;
      //int par = v->l0_compact_indexes[0];    //取出vector中第一个元素 
      //4.24 判断当前第一个compact index是否重复的     
      while (v->l0files[par].size() <= 5){
        ++it;
        par = it->second;
              
      }
      //v->l0_compact_indexes.erase(v->l0_compact_indexes.begin());
      c->par_ = par;
      //std::cout << "picked par:" << par << '\n';    //4.7
      std::vector<FileMetaData*> tmp(v->l0files[par]);
      std::sort(tmp.begin(), tmp.end(), OldestFirst);
